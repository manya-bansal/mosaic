...
static void bench_spmv_mkl_row(benchmark::State& state) {
    ...
    for (int i = 0; i < dim; i++) {
        ...
        if (rand_float < sparsity_new) {
          for (int j = 0; j < dim; j++ ){
              B.insert({i, j}, (float) ((int) (rand_float*3/sparsity_new)));
          }
      }
    }
...
TACO_BENCH(bench_spmv_mkl_row)->DenseRange(10000, 20000, 1000);
