...
string Module::compile() {
  ...
  std::string libPath = "";
  std::string dynamiclibFiles = "";


  char const *ret = getenv("INCLUDE_PATH");
  if (ret) includePath = std::string(ret);
  ret = getenv("LIB_PATH");
  if (ret) libPath = std::string(ret);
  ret = getenv("DYNAMIC_LIB_FILES");
  if (ret) dynamiclibFiles = std::string(ret);
  ...
  cmd += includePath + " " + libPath + " " + dynamiclibFiles;
